        ...
        {!activeChat ? (
          ...
          <div className="flex flex-col h-full w-full">
            ...
            <header className="h-[72px] shrink-0 sticky top-0 z-10 px-4 flex items-center gap-3 bg-[#0a1628] border-b border-[rgba(255,255,255,0.07)]">
              <button onClick={() => setActiveChat(null)} className="md:hidden p-2 -ml-2 text-[var(--teal)] rounded-lg">
            ...
            <footer className="shrink-0 p-4 bg-[#0a1628] border-t border-[rgba(255,255,255,0.07)]">
              <div className="flex items-end gap-3 max-w-5xl mx-auto">
                <div className="flex-1 bg-[#0d1f35] border border-[rgba(255,255,255,0.1)] rounded-xl flex items-end px-4 py-2 focus-within:border-[var(--teal)] transition-colors">
                  <textarea
                    ref={textareaRef}
                    value={input}
                    ...
                    className="flex-1 bg-transparent py-2 text-sm text-[var(--text)] outline-none resize-none max-h-32"
                    rows={1}
                  />
                </div>
                <button 
                  ...
                  className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all ${
                    input.trim() 
                    ? 'bg-gradient-to-r from-[var(--teal)] to-[var(--blue)] text-white shadow-lg' 
...
function HeaderBtn({ icon, onClick }) {
  return (
    <button onClick={onClick} className="w-10 h-10 rounded-xl flex items-center justify-center text-lg hover:bg-[rgba(255,255,255,0.05)] text-[var(--text-dim)] hover:text-[var(--teal)] transition-all">
      {icon}
    </button>
  );
}